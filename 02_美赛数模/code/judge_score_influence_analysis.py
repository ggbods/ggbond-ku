...
def preprocess_data(rank_df, original_df):
    features_df = original_df[['celebrity_name', 'celebrity_age_during_season', 
                              'celebrity_homecountry/region', 'celebrity_industry']].copy()
...
def encode_categorical_features(df):
    X = df[['simulated_vote_share', 'celebrity_age_during_season', 
            'celebrity_homecountry/region', 'celebrity_industry']].copy()
    y = df['avg_judge_score'].copy()
    ...
    numerical_features = ['simulated_vote_share', 'celebrity_age_during_season']
...
def analyze_feature_importance(model, X):
    ...
    feature_importance_df['feature'] = feature_importance_df['feature'].replace({
        'simulated_vote_share': 'Audience Vote Percentage',
...
def visualize_feature_importance(feature_importance_df, title, filename_suffix):
    ...
    plt.tight_layout()
    plt.savefig(f'judge_score_influence_{filename_suffix}.png', dpi=150)
    plt.show()
    
    print(f"\nFeature importance chart saved as judge_score_influence_{filename_suffix}.png")
    ...
    for feature in X.columns:
        ...
        for perturbation in perturbations:
            ...
            sensitivity_results.append({
                'feature': feature,
                'perturbation': perturbation * 100,  # Convert to percentage
...
def visualize_sensitivity_analysis(sensitivity_df, title, filename_suffix):
    ...
    plt.tight_layout()
    plt.savefig(f'judge_score_sensitivity_{filename_suffix}.png', dpi=150)
    plt.show()
    
    print(f"\nSensitivity analysis chart saved as judge_score_sensitivity_{filename_suffix}.png")
...
def visualize_parameter_sensitivity(param_df, title, filename_suffix):
    ...
    plt.tight_layout()
    plt.savefig(f'parameter_sensitivity_judge_{filename_suffix}.png', dpi=150)
    plt.show()
    
    print(f"\nParameter sensitivity chart saved as parameter_sensitivity_judge_{filename_suffix}.png")

def main():
    print("Starting analysis of factors influencing judge scores...")
    ...
    visualize_feature_importance(feature_importance_df, 'Feature Influence on Judge Scores (%)', 'eng')
    
    visualize_feature_importance(feature_importance_df, '评委评分影响因素分析 (%)', '')
    ...
    feature_importance_df.to_csv('judge_score_influence_results.csv', index=False)
    print("\nFeature importance results saved as judge_score_influence_results.csv")
    
    sensitivity_df.to_csv('judge_score_sensitivity_results.csv', index=False)
    print("Sensitivity analysis results saved as judge_score_sensitivity_results.csv")
    
    param_df.to_csv('judge_score_parameter_sensitivity_results.csv', index=False)
    print("Parameter sensitivity analysis results saved as judge_score_parameter_sensitivity_results.csv")
    
    merged_df.to_csv('judge_score_analysis_data.csv', index=False)
    print("Merged data saved as judge_score_analysis_data.csv")
